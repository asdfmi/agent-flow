...
const runManager = new RunManager({
  ...
  validator,
  crawlerFactory: ({ workflow, runId }) => new WorkflowRunner({ workflow, runId, postEvent }),
  logger: console,
});
...
export function startServer() {
  ...
  app.post('/run', async (req, res) => {
    ...
    try {
      ...
      return res.status(500).json({ error: 'crawler failed', message: error?.message || String(error) });
    }
  });
