  ...
  runnerFactory: ({ workflow, execution, runId, startNodeId }) =>
    new WorkflowExecutor({
      ...
      eventDispatcherFactory: ({ runId, logger, publish }) =>
        new RunEventDispatcher({
          runId,
          logger,
          postEvent: publish ?? postEvent,
        }),
      stepHandlers,
...
const runManager = new RunManager({
  ...
  runWorkflow: ({ runId, workflow }) =>
    runWorkflowUseCase.execute({ runId, workflowInput: workflow }),
  logger: console,
});
  ...
  app.post("/run", async (req, res) => {
    ...
    try {
      ...
      if (error instanceof ValidationError) {
        return res
          .status(400)
          .json({ error: error.message, ...(error.metadata ?? {}) });
      }
